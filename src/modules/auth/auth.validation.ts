...
// Validações customizadas (se precisar)
export const customValidations = {
  ...
  passwordStrength: (password: string): { score: number; feedback: string[] } => {
    ...
    if (password.length >= 8) score += 1; else feedback.push('Use pelo menos 8 caracteres');
    if (password.length >= 12) score += 1; else feedback.push('Use pelo menos 12 caracteres para maior segurança');
    if (/[a-z]/.test(password)) score += 1; else feedback.push('Inclua letras minúsculas');
    if (/[A-Z]/.test(password)) score += 1; else feedback.push('Inclua letras maiúsculas');
    if (/\d/.test(password)) score += 1; else feedback.push('Inclua números');
    if (/[^a-zA-Z\d]/.test(password)) score += 1; else feedback.push('Inclua símbolos especiais');
    if (!/(.)\1{2,}/.test(password)) score += 1; else feedback.push('Evite repetir caracteres');
    if (!/123|abc|qwe|password|admin/i.test(password)) score += 1; else feedback.push('Evite sequências ou palavras comuns');
